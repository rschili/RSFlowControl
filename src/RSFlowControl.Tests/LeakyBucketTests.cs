...
public class LeakyBucketTests
{
    ...
    public async Task Constructor_InvalidParameters_ShouldThrowArgumentException()
    {
        ...
        // Act & Assert
        await Assert.That(() => new LeakyBucket(invalidCapacity, 600)).Throws<ArgumentException>();
        await Assert.That(() => new LeakyBucket(10, invalidMaxLeaksPerHour)).Throws<ArgumentException>();
    }
    ...
    {
        // Arrange
        var rateLimiter = new LeakyBucket(10, 600);
    ...
    {
        // Arrange
        var rateLimiter = new LeakyBucket(1, 600);
    ...
    {
        // Arrange
        var rateLimiter = new LeakyBucket(1, 6000);
        rateLimiter.Leak(); // Deplete the quota
